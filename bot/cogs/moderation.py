...
class Mods(commands.Cog):
    ...
    async def recruit(self, ctx, *, sentence):
        ...
        if 516208259099525134 in [y.id for y in authorc.roles] and str(ctx.channel) == "chat-and-commands":
            ...
            for i in range(len(L) - 4):
                ...
                if i != len(L) - 5:
                    notes = notes + " "
            player = str(await self.bot.fetch_user(L[0][3:-1]))
                ...
                for j in range(len(roles)):
                    if tag == roles[j]:
                        ...
                        for i in range(len(W)):
                            ...
                            if i != len(W) - 1:
                                wel = wel + " "
                        channel = await self.bot.fetch_channel(SM[j])
                        await channel.send(f"{wel}")
    ...
    @commands.command()  # Replaces botlog
    @commands.has_permissions(manage_roles=True)
    async def record(self, ctx, *, sentence):
        S = sentence.split(" ")
        if S[0].lower() == "show":
                ...
                if S[0].lower() == rolename[i] and S[1].lower() == "add":
                    for j in range(2, len(S)):
                        ...
                        if S[j] not in LOG[i]:
                            LOG[i].append(S[j])
                        print(LOG)
                        print(await self.bot.fetch_channel(S[j]))
    ...
    @commands.command()  # only roles so far, add branches
    @commands.has_permissions(manage_roles=True)
    async def branch(self, ctx, *, sentence):
        S = sentence.split(" ")
        nn = S[0].lower()
        if nn == "show":
    ...
    @commands.command()
    @commands.has_permissions(manage_roles=True)
    async def welcome(self, ctx, *, sentence):
        S = sentence.split(" ")
        if S[0].lower() == "show":
